Create a single horizontal sprite strip for the Codex app digital pet `life` in the state `running-right`.
...
Output exactly 8 separate animation frames arranged left-to-right in one single row. Each frame must show the same pet: Anthropomorphic retro CRT computer mascot with beige monitor head, dark screen face with simple glowing eyes and smile, blue suit, white shirt, bright red tie, rounded limbs, white gloves, polished black shoes, friendly confident personality..
