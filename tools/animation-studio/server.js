...
const WIN98_CSS_PATH = path.join(
  REPO_ROOT,
  "node_modules",
  "98.css",
  "dist",
...
function listAgents() {
  ...
  return entries
    ...
    .filter(
      (name) =>
        fs.existsSync(agentPath(name, "agent.js")) &&
        fs.existsSync(agentPath(name, "map.png")),
    )
...
function parseDefinitionFromScript(filePath) {
  ...
  if (!captured || typeof captured !== "object") {
    throw new Error(
      `Unable to parse definition from ${path.basename(filePath)}`,
    );
  }
...
function serializeJs(value, indentLevel = 0) {
  ...
  if (Array.isArray(value)) {
    ...
    const parts = value.map(
      (item) => `${nextIndent}${serializeJs(item, indentLevel + 1)}`,
    );
    return `[\n${parts.join(",\n")}\n${indent}]`;
  }
...
function loadAgentPayload(agentName) {
  ...
  const frameSize =
    Array.isArray(definition.framesize) && definition.framesize.length >= 2
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (req.method === "POST" && saveMatch) {
      ...
      sendJson(res, 200, {
        ok: true,
        savedPath: path.relative(REPO_ROOT, result.savedPath),
      });
      return;
    }

    const soundMatch = pathname.match(
      /^\/api\/agent\/([^/]+)\/sound\/([^/]+)$/,
    );
    if (req.method === "GET" && soundMatch) {
      const agentName = decodeURIComponent(soundMatch[1]);
      const soundId = decodeURIComponent(soundMatch[2]);
      const sounds = parseSoundsFromScript(
        agentPath(agentName, "sounds-mp3.js"),
      );
